...
import { Business, Supplier, UnifiedParty, UnifiedLedgerRow } from '../../types.ts';

interface SuppliersSectionProps {
  business: Business;
}

export const SuppliersSection: React.FC<SuppliersSectionProps> = ({ business }) => {
  ...
  const [alsoAsCustomer, setAlsoAsCustomer] = useState(false);
  ...
  const loadSuppliers = async () => {
    setLoading(true);
    try {
      const res = await api.getSuppliers(business.id);
  ...
  const handleCreateSupplier = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      await api.createSupplier(business.id, {
        name,
  ...
  const resetAddForm = () => {
    ...
    setNotes('');
    setOpeningBalance('0');
    setAlsoAsCustomer(false);
  };

  const handleOpenEdit = (supp: Supplier) => {
    setEditingSupplier(supp);
    setEditName(supp.name);
    setEditCompanyName(supp.company_name || '');
    setEditPhone(supp.phone || '');
    setEditEmail(supp.email || '');
    setEditAddress(supp.address || '');
    setEditCity(supp.city || '');
    setEditTaxNumber(supp.tax_number || '');
    setEditNotes(supp.notes || '');
    setEditStatus(supp.status || 'active');
  };
  ...
  const handleUpdateSupplier = async (e: React.FormEvent) => {
    ...
    try {
      await api.updateSupplier(business.id, editingSupplier.id, {
        name: editName,
  ...
  const openUnifiedProfile = async (supp: Supplier) => {
    setActiveProfileSupplier(supp);
    setProfileLoading(true);
    setActionMessage(null);
    try {
      const partyKey = supp.party_id ? String(supp.party_id) : `s_${supp.id}`;
      ...
      if (profileRes && profileRes.party) {
        ...
        setPartyProfile({
          id: supp.party_id || supp.id,
          business_id: business.id,
          customer_id: supp.linked_customer_id || null,
          supplier_id: supp.id,
          name: supp.name,
          ...
          is_customer: Boolean(supp.is_customer || supp.linked_customer_id),
          is_supplier: true,
          customer_balance: 0,
          supplier_balance: supp.current_balance,
          net_balance: -supp.current_balance,
  ...
  const applyLedgerFilter = async () => {
    if (!activeProfileSupplier) return;
    setProfileLoading(true);
    try {
      const partyKey = activeProfileSupplier.party_id
        ? String(activeProfileSupplier.party_id)
        : `s_${activeProfileSupplier.id}`;
  ...
  const handleDesignateAsCustomer = async (suppId: number) => {
    try {
      await api.designateSupplierAsCustomer(business.id, suppId);
      setActionMessage('Successfully designated as Customer. Entity is now Customer + Supplier!');
      loadSuppliers();
      if (activeProfileSupplier && activeProfileSupplier.id === suppId) {
        openUnifiedProfile(activeProfileSupplier);
      }
    } catch (err: any) {
      alert(err.message || 'Failed to designate as Customer');
    }
  };

  const handleRemoveCustomerRole = async (suppId: number) => {
    if (!confirm('Are you sure you want to remove Customer role from this entity? Historical transactions remain safe in the unified ledger.')) {
      return;
    }
    try {
      await api.removeSupplierRole(business.id, suppId);
      setActionMessage('Customer role removed.');
      loadSuppliers();
      if (activeProfileSupplier && activeProfileSupplier.id === suppId) {
        openUnifiedProfile(activeProfileSupplier);
      }
  ...
  const handleDeleteClick = (supp: Supplier) => {
    setPartyToDelete(supp);
    setDeleteError(null);
  };

  const confirmDeleteSupplier = async (force: boolean = false) => {
    if (!partyToDelete) return;
    try {
      const partyKey = partyToDelete.party_id ? String(partyToDelete.party_id) : `s_${partyToDelete.id}`;
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Maintain unified supplier & vendor profiles, dual-role entities, and running ledgers
          </p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            <thead className="bg-slate-50 text-slate-700 font-semibold border-b border-slate-200">
              <tr>
                <th className="py-3.5 px-4">Entity / Supplier Name</th>
            ...
            <tbody className="divide-y divide-slate-100 text-slate-800">
              {filteredSuppliers.map((supp) => {
                const isDualRole = Boolean(supp.is_customer || supp.linked_customer_id);
                return (
                  <tr key={supp.id} className="hover:bg-slate-50/70 transition-colors">
                    <td className="py-3.5 px-4 font-bold text-slate-900">
                      <div>{supp.name}</div>
                      {supp.company_name && (
                        <div className="text-xs font-normal text-slate-500 flex items-center space-x-1 mt-0.5">
                          <Building2 className="w-3 h-3 text-slate-400" />
                          <span>{supp.company_name}</span>
                        </div>
                      )}
                    </td>
                    ...
                    <td className="py-3.5 px-4 text-slate-600 max-w-xs truncate text-xs">
                      <div>{supp.address || '-'}</div>
                      {supp.city && <div className="text-slate-400">{supp.city}</div>}
                    </td>

                    <td className="py-3.5 px-4 text-right font-mono font-bold">
                      <span
                        className={
                          supp.current_balance > 0
                      ...
                      >
                        {business.currency} {supp.current_balance.toFixed(2)}
                      </span>
                    </td>
                    ...
                    <td className="py-3.5 px-4 text-center">
                      <div className="inline-flex items-center space-x-1.5">
                        <button
                          onClick={() => openUnifiedProfile(supp)}
                        ...
                        {!isDualRole ? (
                          <button
                            onClick={() => handleDesignateAsCustomer(supp.id)}
                            title="Also designate this entity as Customer"
                            className="px-2 py-1.5 bg-blue-50 border border-blue-200 hover:bg-blue-100 text-blue-700 rounded-lg text-xs font-semibold inline-flex items-center space-x-1 transition-colors"
                          >
            ...
            <form onSubmit={handleCreateSupplier} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. ABC Suppliers or Steel Mills Corp"
              ...
              <div className="p-3.5 bg-indigo-50/70 border border-indigo-200/80 rounded-xl">
                <label className="flex items-start space-x-3 cursor-pointer">
                  <input
                    type="checkbox"
                    id="also-customer-checkbox"
                    checked={alsoAsCustomer}
                    onChange={(e) => setAlsoAsCustomer(e.target.checked)}
                    className="mt-1 w-4 h-4 text-indigo-600 rounded border-slate-300 focus:ring-indigo-500"
                  />
                  <div>
                    <span className="font-semibold text-sm text-slate-900">
                      Also designate this supplier as a Customer
                    </span>
                    <p className="text-xs text-slate-600 mt-0.5">
                      Creates a single unified party entity that appears in both Supplier and Customer lists, sharing ONE combined ledger for purchase bills, sales invoices, and payments.
                    </p>
                  </div>
                </label>
              </div>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl max-w-5xl w-full p-6 text-slate-900 shadow-2xl max-h-[92vh] flex flex-col">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-4 border-b border-slate-100 shrink-0 gap-3">
              <div>
                <div className="flex items-center space-x-3">
                  <h3 className="text-xl font-bold text-slate-900 flex items-center space-x-2">
                    <span>{partyProfile?.name || activeProfileSupplier.name}</span>
                  </h3>
              ...
              <div className="flex items-center space-x-2">
                {!partyProfile?.is_customer && (
                  <button
                    onClick={() => handleDesignateAsCustomer(activeProfileSupplier.id)}
                    className="px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-semibold transition-colors flex items-center space-x-1.5"
                  >
                    <Plus className="w-3.5 h-3.5" />
                    <span>Add as Customer</span>
                  </button>
                ...
                {partyProfile?.is_customer && partyProfile?.is_supplier && (
                  <button
                    onClick={() => handleRemoveCustomerRole(activeProfileSupplier.id)}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 py-3 border-b border-slate-100 text-xs shrink-0">
              ...
              <div className="p-2.5 bg-indigo-50/70 rounded-xl border border-indigo-100">
                ...
                >
                  {business.currency} {(partyProfile?.net_balance ?? -activeProfileSupplier.current_balance).toFixed(2)}
                </span>
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 py-3 shrink-0">
              ...
              <div className="text-xs text-slate-500 italic">
                One entity &bull; One unified ledger (Purchase Bills, Sales Invoices & Payments)
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 rounded-2xl max-w-md w-full p-6 text-slate-900 shadow-2xl">
            ...
            <p className="text-sm text-slate-600 mb-4">
              If this entity has existing bills, invoices, or payments, deleting it completely would damage audit history. You may instead deactivate it or remove specific roles.
            </p>

            {partyToDelete.is_customer && (
              <div className="mb-4 p-3 bg-purple-50 border border-purple-200 rounded-xl text-xs text-purple-900">
                <strong>Notice:</strong> This entity is also designated as a Customer. You can remove the Customer role or Supplier role individually to preserve records.
              </div>
            )}
            ...
            <div className="flex flex-col space-y-2 pt-2 border-t border-slate-100">
              <button
                type="button"
                onClick={() => confirmDeleteSupplier(false)}
